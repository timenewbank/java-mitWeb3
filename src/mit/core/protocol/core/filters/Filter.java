...
import mit.core.protocol.core.RpcErrors;
...
public abstract class Filter<T> {
    ...
    public void run(ScheduledExecutorService scheduledExecutorService, long blockTime) {
        try {
            MitFilter ethFilter = sendRequest();
    ...
    private void getInitialFilterLogs() {
        try {
            Optional<Request<?, MitLog>> maybeRequest = this.getFilterLogs(this.filterId);
            MitLog ethLog = null;
    ...
    private void pollFilter(MitFilter ethFilter) {
        MitLog ethLog = null;
        try {
            ethLog = web3j.mitGetFilterChanges(filterId).send();
        } catch (IOException e) {
            throwException(e);
        }
        if (ethLog.hasError()) {
    ...
    abstract MitFilter sendRequest() throws IOException;

    abstract void process(List<MitLog.LogResult> logResults);
    ...
    public void cancel() {
        ...
        try {
            MitUninstallFilter ethUninstallFilter = web3j.mitUninstallFilter(filterId).send();
     ...
     */
    protected abstract Optional<Request<?, MitLog>> getFilterLogs(BigInteger filterId);
