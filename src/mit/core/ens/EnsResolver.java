package mit.core.ens;

import mit.crypto.WalletUtils;
import mit.core.ens.contracts.generated.ENS;
import mit.core.ens.contracts.generated.PublicResolver;
...
import mit.core.tx.ClientTransactionManager;
import mit.core.tx.ManagedTransaction;
import mit.core.tx.TransactionManager;
import mit.core.tx.gas.DefaultGasProvider;
...
public class EnsResolver {
    ...
    boolean isSynced() throws Exception {
        MitSyncing ethSyncing = web3j.mitSyncing().send();
        if (ethSyncing.isSyncing()) {
            return false;
        } else {
            MitBlock ethBlock =
                    web3j.mitGetBlockByNumber(DefaultBlockParameterName.LATEST, false).send();
